...
BASE=/Volumes/Bennett_BACKUP/Research/jensen-metabolites/fiji_sal-microscale
...
do
	...
	cat $BASE/blobplot/$genome.bowtie.blobplot.txt | awk '(NR>1) && ($4 > 20 )' | cut -f1 > $genome'_temp.txt'
	...
	done < $BASE/spades/$genome.fix_ids.txt
...
ani-aai-matrix.sh -m ani -i .fasta -t 8
